"""
Training and evaluation functions for text classification models.

train() and train_two_sents(): Train a text classification model on a dataset, return loss and accuracy.

train_EP(): Train a text classification model using Embedding Poisoning.

evaluate() and evaluate_two_sents(): Evaluate a model on a dataset, return loss, accuracy and F1 score.

train_iter(), train_EP_iter(): Single training iteration.

binary_accuracy(): Compute accuracy for binary classification.
"""
import random
import numpy as np
import torch
...
def train(model, parallel_model, tokenizer, train_text_list, train_label_list,
    ...
    return float(epoch_loss) / total_train_len, float(epoch_acc_num) / total_train_len
...
def evaluate_two_sents(model, tokenizer, eval_sent1_list, eval_sent2_list, eval_label_list, batch_size, criterion, device):
    ...
    return epoch_loss / total_eval_len, epoch_acc_num / total_eval_len, f1_score(true_labels, predict_labels, average="macro")